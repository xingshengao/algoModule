...
void solve() {
    ...
    vector<int> ans(m, 0); // 每种猎人最多可击杀的猎物的数量
    
    // 猎物的set
    vector<set<char>> vec;
    for (int i = 0; i < n; ++i) {
        set<char> tmp(all(b[i]));
        vec.push_back(tmp);
    }


    for (int i = 0; i < m; ++i) {
        set<char> S(all(a[i])); // 猎人技能
        //cout << S << endl;
        // 猎人技能不包括A-F，则数量为0
        if (!S.count('A') && !S.count('B') && !S.count('C') && !S.count('D') && !S.count('E') && !S.count('F')) {
            continue;
        }
        for (int j = 0; j < n;++j) {
            if (!vec[j].count(a[i][7])) continue; // 猎人最后一个技能不能命中任意一个弱点
