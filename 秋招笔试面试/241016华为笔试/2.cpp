...
void solve() {
    ...
    function<bool(int, int)> is = [&](int i, int j) -> bool {
        stack<int> st;
        ...
        for (int j = i + 1; j < n; ++j) {
            ...
            for (int a = j + 1; a < n; ++a) {
                for (int b = a + 1; b < n; ++b) {
                    if (f[a][b]) {
                        string ss = "";
                        for (int k = 0; k < i; ++k) ss.push_back(t[k]);
                        for (int k = a; k <= b; ++k) ss.push_back(t[k]);
                        for (int k = j + 1; k < a; ++k) ss.push_back(t[k]);
                        for (int k = i; k <= j; ++k) ss.push_back(t[k]);
                        for (int k = b + 1; k < n; ++k) ss.push_back(t[k]);
                        if (ans < ss) ans = ss;
                    }
