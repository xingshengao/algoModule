...
LL memo[15][127][127];  // pos, 余数, sum
LL calc(string s, LL d) {
    LL m = s.size();
    memset(memo, -1, sizeof(memo));
    function<LL(LL, LL, LL, bool, bool)> dfs = [&](LL i, LL v, LL sum, bool is_limit, bool is_num) -> LL {
        if (i == m) {
            return (int)(is_num and v == 0 and sum == d);
        }
        if (!is_limit and is_num and memo[i][v][sum] != -1) return memo[i][v][sum];
        LL res = 0;
        if (!is_num) res += dfs(i + 1, (v * 10) % d, sum, false, false);
        int up = is_limit ? s[i] - '0' : 9;
        int down = is_num ? 0 : 1;
        for (int k = down; k <= up; ++k) {
            res += dfs(i + 1, (v * 10 + k) % d, sum + k, is_limit and k == up, true);
        }
        if (!is_limit and is_num) return memo[i][v][sum] = res;
        return res;
    };
