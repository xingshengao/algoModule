...
struct Primes {
    ...
    bool isprime(long long x) {
        if (x < 2) return false;
        if (x < (int)lpf.size()) return lpf[x] == x;
...
// 塔子哥拿到了n个正整数 ，她希望你求出这些数的阶乘全部来在一起生成的大数有多少个因子。你能帮帮她吗
void solve() {
    int MX = 1e6 + 10;
    int n;
    cin >> n;
    ...
    VI cnt(MX + 2);  // 最终阶乘中每个数字出现了多少次
    int s = 0;
    for (int i = 0; i < MX; ++i) {
        s += d[i];
        cnt[i] = s;
    }
    unordered_map<int, int> mp;  // 存每个质因子出现的次数
    int mx = MAX(a);
    for (int i = 1; i <= mx; ++i) {
        for (auto& [k, v] : primes.factorise(i)) {
            mp[k] += v * cnt[i];
            mp[k] = (mp[k] % mod + mod) % mod;
        }
    }
    // debug(mp);
    LL ans = 1LL;
    // 每个质因子都可以选[0:v]个数，共有v+1种选择方法
    for (auto& [k, v] : mp) {
        ans = ans * (v + 1) % mod;
        ans = (ans % mod + mod) % mod;
    }
