...
// 小苯有一个长度为n
// 的数组 a，小红有一个长度为m的数组 b 小苯希望对 中的元素进行一些变换，使得 的所有元素都在 中出现过。具体的，他可以做任意如下操作 : 选择
// ，令 或.小苯想在变换后，同时又能使得a单调不降，他想知道他最少需要操作多少次，请你帮帮他吧。
// 数据级别1000
void solve() {
    int n, m;
    cin >> n >> m;
    ...
    sort(b.begin(), b.end());
    // dp[i][j]是a[i + 1]变成b[j + 1]的最小次数，且保持单调非递减
    vector dp(n + 1, vector<int>(m + 1, 1e18));
    dp[0][0] = 0;
    for (int j = 1; j <= m; ++j) dp[0][j] = 0;
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            dp[i][j] = min(dp[i - 1][j - 1], dp[i - 1][j]) + abs(a[i - 1] - b[j - 1]);
        }
    }
    LL ans = 1e18;
