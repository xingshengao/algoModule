...
void solve() {
    ...
    function<TreeNode*(int, int)> make_root = [&](int l, int r) -> TreeNode* {
        if (l > r) return nullptr;
        int mid = l + (r - l >> 1);
        TreeNode* node = new TreeNode(arr[mid]);
        node->left = make_root(l, mid - 1);
        node->right = make_root(mid + 1, r);
        return node;
    };
    TreeNode* root = make_root(0, arr.size() - 1);
