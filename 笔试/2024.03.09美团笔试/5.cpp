...
// 题目描述
// 小美认为，在人际交往中，但是随着时间的流逝，朋友的关系也是会慢变淡的，最终朋友关系就淡忘了
// 现在初始有一些朋友关系，存在一些事件会导致两个人淡忘了他们的朋友关系。小美想知道某一时刻中，某两人是否可以通过朋友介绍互相认识?

// 事件共有2种: 1u v:代表编号u的人和编号v的人淡忘了他们的朋友关系。 2 u v:代表小美查询编号u的人和编号v的人是否能通过朋友介绍互相认识.
// 注:介绍可以有多层，比如2号把1号介绍给3号，然后3号再把1号介绍给4号，这样1号和 4 号就认识了。
struct DSU {
    vector<int> f, siz;
    DSU() {}
    DSU(int n) { init(n); }
...
void solve() {
    int n, m, q;
    cin >> n >> m >> q;
    // 找出所有可能要删除的边, 假设所有边都被删除了, 构建一个最终的并查集
    // 逆向遍历q次操作, 如果是查询, 则查出, 如果是删除, 则往并查集添加边即可
    set<PII> s;  // 存初始边
    for (int i = 0; i < m; ++i) {
        int a, b;
        cin >> a >> b;
    ...
    for (int qi = 0; qi < q; ++qi) {
        ...
        --u, --v;
        if (u > v) swap(u, v);
        vec[qi][0] = op, vec[qi][1] = u, vec[qi][2] = v;
        if (op == 1) {
            auto aa = PII(u, v);
            if (s.count(aa)) {
                s.erase(aa);
    ...
    vector<string> ans;
    for (int i = q - 1; i >= 0; --i) {
        int op = vec[i][0], u = vec[i][1], v = vec[i][2];
        if (u > v) swap(u, v);
        if (op == 2) {
            if (dsu.same(u, v))
                ans.push_back("Yes");
