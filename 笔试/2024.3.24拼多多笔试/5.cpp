...
// 塔子哥可以对这个字符串进行多次操作,每次操作可以把该字符串中一段连续的回文子串删除(单个字符也属于回文串)
// 删除后剩下的串会拼在一起,请问最少需要多少次操作可以将这个字符串删光
void solve() {
    string s;
    cin >> s;
    int n = s.size();
    VVI memo(n + 1, VI(n + 1, 1e18));
    ...
    for (int i = 0; i < n; ++i) for (int j = i; j < n; ++j) if (g[i][j]) memo[i][j] = 0;
    
    // dfs(i, j)代表把[i: j]变为回文串的最小操作次数
