...
// n个数字，Alice去掉d个，Bob剩下的数字最多m个乘以-k，Alice想大，Bob想小，最终剩余的数字之和是多少
void solve() {
    int n, m, k, d;
    cin >> n >> m >> k >> d;
    vector<int> a(n);
    ...
    // Alice肯定是删除最大的几个，BOB肯定是乘最大的几个-k，枚举Alice删除几个
    int ans = -1e18;
    VI ps(n + 1);
    ...
    for (int j = 0; j <= d; ++j) {
        ...
        if (n - 1 - j + 1 <= m) {
            ...
            // [j, j + m - 1]乘-k，其余[j + m, n - 1]不变
            cur = -k * (ps[j + m] - ps[j]) + ps[n] - ps[j + m];
        }
        chmax(ans, cur);
    }
    cout << ans << endl;
